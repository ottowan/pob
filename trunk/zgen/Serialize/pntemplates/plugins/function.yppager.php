<?php
// $Id: function.yppager.php,v 1.1 2010/12/01 12:11:07 demobile Exp $
// ----------------------------------------------------------------------
 ...
 ***    @subpackage   pnRender
 ***    @version      $Id: function.yppager.php,v 1.1 2010/12/01 12:11:07 demobile Exp $
...
/**
* http://www.smarty.hu/plugins/duda/function.yppager.php
* Smarty plugin
* -------------------------------------------------------------
* Type:     function
* Name:     pager
...
*
* Example:
* <--[yppager show="page" rowcount=$pager.numitems limit=$pager.itemsperpage posvar=startnum shift=1]-->
*
...
*/
function smarty_function_yppager($params, &$smarty) {
    // START INIT
    $show         = 'record';
    $posvar       = 'pos';
    $separator    = '&#150;'; // En-dash: &#151; is em-dash
    $class_text   = 'nav';
    $class_num    = 'small';
    $class_numon  = 'big';
    $txt_pos      = 'middle';
    ...
    if (!empty($Request['module'])) {
    	...
    	$type = (!empty($Request['type']) ? $Request['type'] : 'user');
        $func = (!empty($Request['func']) ? $Request['func'] : 'main');
...
//    $modurl = (($modinfo['type'] == 2 || $modinfo['type'] == 3) && pnConfigGetVar('ShortURLs'));
    $modurl = pnConfigGetVar('ShortURLs') && ($type == 'user');
    if ($modurl) {
    	unset($vars['module']); unset($vars['type']); unset($vars['func']);
    } else { // Non-API URLs
        $query = 'module='.$module.'&func='.$func.($type!='user' ? '&type='.$type : '');
        foreach($vars as $key=>$value)    {
            if (is_array($value))    {
                foreach($value as $skey => $svar) {
                    $query .= '&'.$key."[$skey]=".urlencode($svar);
                }
            } elseif(!empty($value)) {
    ...
    if (($pos == 0) AND ((!empty($txt_first)) OR !empty($img_first))){
        ...
        if (!empty($img_first))    {
            if (strpos($img_first, '<')===true) { // preg_match('/<img/i', $img_first)
                ...
                // image url
                print '<img align="absmiddle" src="'.$img_first.'" border="0" />';
            }
        } else    {
            print $txt_first;
        }
        ...
        if ($pos >= $limit)    {
            ...
            if (!empty($img_prev))    {
                if (strpos($img_prev, '<')===true) { // preg_match('/\</', $img_prev)
                    ...
                    // image url
                    $cache['prev'] .= '<img align="absmiddle" src="'.$img_prev.'" border="0" />';
                }
            } else    {
                $cache['prev'] .= $txt_prev;
            }
        ...
        if ($pos < ($rowcount-$limit))    {
            ...
            if (!empty($img_next))    {
                if (preg_match('/\</', $img_next)) {
                    ...
                    // image url
                    $cache['next'] .= '<img align="absmiddle" src="'.$img_next.'" border="0" />';
                }
            } else    {
                $cache['next'] .= $txt_next;
            }
        ...
        // START PRINTOUT
        if ($txt_pos == 'top')    {
            print '<p style="text-align:center; text-decoration: none;">'.$cache['prev'].$cache['next'].'</p>'."\n";
        }
        print '<p style="text-align:center" class="pager">';
        ...
        foreach($pages as $num=>$url)    {
               ...
               if ($num != $short['now']) { // Don't have a link to the current page
                        print '<a class="'.$class_num.'" href="'.htmlspecialchars($url).'">'.$tmp.'</a>';
               } else { print '<span class="'.$class_numon.'">'.$tmp.'</span>';
               }
        }
    ...
    SessionUtil::setVar('row_count',$rowcount);
    SessionUtil::setVar('limit',$limit);
